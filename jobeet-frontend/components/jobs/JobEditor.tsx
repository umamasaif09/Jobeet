  ...
  async function submitJob() {
    ...
    try {
      ...
      if (mode == "create") {
        ...
        if (userType == "public") {
          await updateJob(jobId!, token!, request);
          toast.success("Job updated successfully");
          router.replace(`/jobs/${jobId}`);
        } else {
          await adminUpdateJob(jobId!, request);
          toast.success("Job updated successfully");
          router.replace("/admin/jobs");
        }
      }
      ...
      toast.warning(
        mode == "create" ? "Unable to create job." : "Unable to update job.",
      );
    }
  }
    ...
    return (
      <>
        <div className="flex items-baseline gap-4 my-[24px]">
          <BackButton />
          <h1 className={pageStyles.pageTitle}>Job Created</h1>
        </div>
          ...
          <CardContent className="py-[9px] px-[24px]">
            <div className="space-y-4">
              ...
              <p className="text-[16px]">
                Save this link to edit your job later:{" "}
              </p>

              <Link
                href={createdJob.editLink}
                className="text-[#3498db] hover:underline cursor-pointer text-[16px]"
              >
                {createdJob.editLink}
              </Link>
            </div>
